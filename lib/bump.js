...
const increment = async (type, preSuffix, webextDir) => {
	...
	const webextPath = path.join(process.cwd(), webextDir, 'manifest.json');

	if (!fs.existsSync(webextPath)) {
		throw new Error(`"${webextPath}" not exist`);
	}

	let webextContent;

	try {
		webextContent = await fs.readJSON(webextPath);
	} catch (err) {
		throw new Error(`Couldn't parse "${webextPath}"`);
	}

	webextContent.version = newVersion;

	try {
		await fs.writeJSON(webextPath, webextContent, {
			spaces: 2
		});
	} catch (err) {
		throw new Error(`Couldn't write to "${webextPath}"`);
	}
...
module.exports = async (type, preSuffix, webextDir) => {
	createSpinner('Bumping version tag');
	let version;

	try {
		version = await increment(type, preSuffix, webextDir);
	} catch (err) {
		fail(err.message);
	}
